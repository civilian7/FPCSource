...
# 소스 / 중간산출물 / 실행파일을 갈라 둔다 - LumiPascal 트리와 같은 규약.
#   compiler\  소스만
#   obj\       .ppu / .o
#   bin\       fcc64.exe
$Obj    = Join-Path $Root 'obj\compiler'
$Bin    = Join-Path $Root 'bin'
$Output = Join-Path $Bin 'fcc64.exe'
...
if ($Clean) {
    ...
    # 예전 빌드가 소스 옆에 남긴 것도 함께 치운다.
    Get-ChildItem $Compiler -Include *.ppu, *.o -Recurse -ErrorAction SilentlyContinue |
        Remove-Item -Force
}

New-Item -ItemType Directory -Path $Obj, $Bin -Force | Out-Null
...
if (-not (Test-Path $MsgTxt) -or
    ...
    try {
        # msg2inc 도 소스 옆을 더럽히지 않는다 - 실행파일·유닛 모두 obj 로.
        & $Bootstrap "-FE$Obj" "-FU$Obj" utils\msg2inc.pp 2>&1 |
            Select-String 'Fatal|Error:' | Out-Host
        & (Join-Path $Obj 'msg2inc.exe') msg\errore.msg msg msg 2>&1 | Out-Null
...
try {
    ...
    # -FU 는 유닛(.ppu/.o) 출력 디렉터리, -FE 는 실행파일 출력 디렉터리.
    # 둘을 주지 않으면 컴파일러가 각 소스 옆에 .ppu/.o 를 흩뿌린다.
    $fa = @(
        '-dx86_64', '-dGDB',
        '-Fux86_64', '-Fux86', '-Fusystems',
        '-Fix86_64', '-Fix86', '-Fisystems', '-Fimsg',
        "-FU$Obj", "-FE$Obj", '-ofcc64'
    ) + $narrow
    ...
    # -o 가 확장자를 떼므로 되돌린다. -FE 로 보냈으니 obj 에서 꺼내 온다.
    foreach ($cand in @((Join-Path $Obj 'fcc64'), (Join-Path $Obj 'fcc64.exe'))) {
        if (Test-Path $cand) {
            Move-Item $cand $Output -Force
            break
        }
    }
